...
struct config {
#define X(type, name, default_, unused) type name = {default_};
  ALL_SETTINGS
#undef X
};
...
config parse_cmd_line_settings(int argc, char *argv[])
{
  auto opts = cxxopts::Options(
      "scorer", "Calculates the overlap between gridify output and a ligand");

  opts.add_options()
#define X(type, name, default_, desc) (#name, desc, cxxopts::value<type>()->default_value(#default_))
      ALL_SETTINGS
#undef X
  ;
...
#define X(type, name, default_, desc)                \
